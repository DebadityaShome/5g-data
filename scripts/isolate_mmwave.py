...
if len(os.listdir(path))>0:
	...
	os.system("mv '"+path+"' '"+path+"/../mmwave_old_logs/"+str(dt.year)+str(dt.month)+str(dt.day)+str(dt.hour)+str(dt.minute)+str(dt.second)+"'")
	os.system("mkdir -p '"+path+"'")
...
for i in range(0,numUe):
	if os.path.isfile(PositionsFile_trace+str(i)+".csv"):
		os.system("mv '"+PositionsFile_trace+str(i)+".csv' '"+orig_dest+"'")
...
if os.path.isfile(RxPacketTraceFile):
	with open(RxPacketTraceFile, 'rb') as trace:
		...
		tracereader = csv.reader((x.replace('\0','') for x in trace),delimiter=',')
		
		headings = str(next(tracereader))[1:-1]
		for rnti in range(1,numUe):
			uefile=open(path+"/Isolated/UE_"+str(rnti)+"_isolated_DL.csv","w+")
			uefile.write(headings)
			uefile.close()
			uefile=open(path+"/Isolated/UE_"+str(rnti)+"_isolated_UL.csv","w+")
			uefile.write(headings)
			uefile.close()
			
		for cellid in range(1,numEnb):
			enbfile=open(path+"/Isolated/ENB_"+str(cellid)+"_isolated_DL.csv","w+")
			enbfile.write(headings)
			enbfile.close()
			enbfile=open(path+"/Isolated/ENB_"+str(cellid)+"_isolated_UL.csv","w+")
			enbfile.write(headings)
			enbfile.close()
...
#			rowcount+=1
			if row[0] == "DL":
				rnti = row[7]
				cellid = row[6]
				enbfile = open(path+"/Isolated/ENB_"+cellid+"_isolated_DL.csv","a+")
				enbfile.write("\n"+str(row)[1:-1])
				uefile = open(path+"/Isolated/UE_"+rnti+"_isolated_DL.csv","a+")
				uefile.write("\n"+str(row)[1:-1])
			elif row[0] == "UL":
				rnti = row[7]
				cellid = row[6]
				enbfile = open(path+"/Isolated/ENB_"+cellid+"_isolated_UL.csv","a+")
				enbfile.write("\n"+str(row)[1:-1])
				uefile = open(path+"/Isolated/UE_"+rnti+"_isolated_UL.csv","a+")
				uefile.write("\n"+str(row)[1:-1])
			uefile.close()
			enbfile.close()
else:
	print "ERROR: Log File "+RxPacketTraceFile+" has not been generated. Cannot Parse."
...
for i in range(0,numUe-1):
	with open(path+"/Originals/mmwavePositions"+str(i)+".csv") as ueposlog:		
		with open(path+"/Isolated/UE_"+str(i+1)+"_isolated_DL.csv",'rb') as uerxlog:
			...
			headings=str(next(rxreader))[1:-1] +","+ str(next(posreader))[1:-1]+"\n"
			dlfile = open(path+"/Complete/DL/UE_"+str(i)+"_interval_"+str(interval)+".csv",'w+')
			...
			for row in ueposlog:
				...
				if endofrx:
					dlfile.write("DL,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA," + row)
				else:
					if getNext:
					...
					if posTime<rxTime and abs(posTime - rxTime)>interval:
						...
						if nextRx == 'eof':
							...
							nextRxTime = float(nextRx.split(",")[1].replace("'",""))
	...
	with open(path+"/Originals/mmwavePositions"+str(i)+".csv") as ueposlog:		
		...
		with open(path+"/Isolated/UE_"+str(i+1)+"_isolated_UL.csv",'rb') as uerxlog:
			...
			posreader = csv.reader(ueposlog,delimiter=',')
			headings=str(next(rxreader))[1:-1] +","+ str(next(posreader))[1:-1]
			ulfile = open(path+"/Complete/UL/UE_"+str(i)+"_interval_"+str(interval)+".csv",'w+')
